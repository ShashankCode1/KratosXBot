...
from KratosXBot import DRAGONS, dispatcher
from KratosXBot.modules.disable import DisableAbleCommandHandler
from KratosXBot.modules.helper_funcs.admin_rights import user_can_changeinfo
from KratosXBot.modules.helper_funcs.alternate import send_message
from KratosXBot.modules.helper_funcs.chat_status import (
    ADMIN_CACHE,
    bot_admin,
    can_pin,
    can_promote,
...
from KratosXBot.modules.helper_funcs.extraction import (
    extract_user,
    extract_user_and_text,
)
from KratosXBot.modules.log_channel import loggable
...
def setchatpic(update: Update, context: CallbackContext):
    ...
    if msg.reply_to_message:
        if msg.reply_to_message.photo:
            ...
            msg.reply_text("You can only Set Photo as Group PFP!")
            return
...
def setchat_title(update: Update, context: CallbackContext):
    ...
    if user_can_changeinfo(chat, user, context.bot.id) is False:
        msg.reply_text("You Dont' have Permissions to Change Group Info Baby!")
        return
...
def promote(update: Update, context: CallbackContext) -> str:
    ...
    if user_id == bot.id:
        message.reply_text(
            "I can't Promote Myself, My Owner Disn't told me to do so."
        )
        return
...
def lowpromote(update: Update, context: CallbackContext) -> str:
    ...
    if not user_id:
        message.reply_text(
            "I don't who's that User, Never seen him in any of the chats where I am Present!",
        )
        return
...
def demote(update: Update, context: CallbackContext) -> str:
    ...
    if user_id == bot.id:
        message.reply_text("I cannot demote myself, but if you want, I can leave.")
        return
...
def set_title(update: Update, context: CallbackContext):
    ...
    if not user_id:
        message.reply_text(
            "I don't know who that user is, never seen him in any of the chats where I am present!",
        )
        return

    if user_member.status == "creator":
        message.reply_text(
            "That user is the owner of the chat and I don't want to put myself in danger.",
        )
        return
    ...
    if user_id == bot.id:
        message.reply_text(
            "I can't set title for myself, my owner didn't tell me to do so.",
        )
        return

    if not title:
        message.reply_text(
            "You think that setting a blank title will change something?"
        )
        return
    ...
    try:
        ...
        message.reply_text(
            "Maybe that user is not promoted by me or maybe you sent something that can't be set as a title."
        )
        return
...
def pin(update: Update, context: CallbackContext) -> str:
    ...
    if prev_message and is_group:
        try:
            ...
            msg.reply_text(
                f"Successfully pinned that message. Click on the button below to see the message.",
