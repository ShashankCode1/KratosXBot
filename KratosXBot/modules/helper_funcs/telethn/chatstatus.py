...
from KratosXBot import DRAGONS
from KratosXBot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def kratos_is_admin(chat_id: int):
    status = False
    kratos = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if kratos.id == user.id:
            status = True
            break
    return status
